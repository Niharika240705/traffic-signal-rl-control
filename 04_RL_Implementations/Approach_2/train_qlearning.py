"""
Approach 2 — Q-learning (off-policy TD control)

Update: Q(S,A) <- Q(S,A) + α [ R + γ max_a Q(S',a) - Q(S,A) ]
The target is the greedy (optimal) action-value, while behaviour remains
ε-greedy. This matches a traffic desk that explores in simulation but wants
to deploy a fully greedy signal plan.
"""
...
def main():
    ...
    q, rewards, extra = train_qlearning()
    save_artifacts(out, "Q-learning", q, rewards, extra)
    print(f"Q-learning done in {extra['wall_time_sec']:.1f}s | last-100 mean {rewards[-100:].mean():.2f}")
